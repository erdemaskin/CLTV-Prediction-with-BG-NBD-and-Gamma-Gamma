...
# TASK 2: Creating the CLTV Data Structure
           ...
           # Create a new cltv dataframe with the values ​​2.customer_id, recency_cltv_weekly, T_weekly, frequency
           # and monetary_cltv_avg.
           # Monetary value will be expressed as average value per purchase, recency and tenure values ​​will be
           # expressed in weekly terms.
...
# TASK 3: BG/NBD, Establishing Gamma-Gamma Models, Calculating CLTV
           ...
           # 2. Fit the Gamma-Gamma model. Estimate the average value of the customers and add it to the cltv dataframe
...
# TASK 4: Creating Segments by CLTV
           # 1. Divide all your customers into 4 groups (segments) according to the 6-month standardized CLTV and add
                # the group names to the dataset. Add it to the dataframe with the name cltv_segment.
...
df_ = pd.read_csv("week_3/flo_data_20k.csv")
df = df_.copy()
df.head()
...
df["order_num_total_ever_offline"].max()
for col in columns:
...
# Create a new cltv dataframe containing the 2.customer_id, recency_cltv_weekly, T_weekly, frequency and monetary_cltv_avg values.
...
# rfm["recency"] = (analysis_date - df["last_order_date"]).astype('timedelta64[D]')  rfm definition
# cltv_df["recency_cltv_weekly"] = ((df["last_order_date"] - df["first_order_date"]).astype('timedelta64[D]')) / 7  cltv definition
cltv_df.head()
...
# 3. Make short 6-month action suggestions to the management for 2 groups you will choose from among 4 groups.
